...
const titulo = document.querySelector('#createdBy')

titulo.innerHTML = 'Aplicación creada por Mauricio'
...
function buildToDo(todo, index) {
  ...
  check.classList.add('completeCheckbox')
...
function buildToDos(toDos) {
  // Tu código acá:
  let newtodo = toDos.map(buildToDo,toDos)
  return newtodo
}
...
function displayToDos() {
  ...
  let lista = buildToDos(toDoItems)
  for(let i=0;i<lista.length;i++){
    toDoContainer.appendChild(lista[i])
  }
